...
def predict_test_images(model, test_filenames, test_images):
    y_pred = model.predict(test_images)
    y_pred = np.argmax(y_pred, axis=0)
    with open("1705056_prediction.csv", "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["FileName", "Digit"])
        for i in range(len(test_filenames)):
            writer.writerow([test_filenames[i], y_pred[i]])


if __name__ == "__main__":
    num_class = 10
    best_model = Model()
    with open("model.pickle.pickle", "rb") as f:
        best_model = pickle.load(f)

    print("Test directory:", sys.argv[1])
    test_filenames, test_images = load_test_data(sys.argv[1])
    print("Test images shape:", test_images.shape)
    predict_test_images(best_model, test_filenames, test_images)
